...
public class AdapterServerPlaylist extends RecyclerView.Adapter {
    ...
    public AdapterServerPlaylist(Context context, ArrayList<Object> arrayList) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MyViewHolder) {
            ...
            ItemServerPlayList item = (ItemServerPlayList) arrayList.get(position);
...
//                            CardView cardView = (CardView) ((Activity) context).getLayoutInflater().inflate(R.layout.layout_native_ad_admob, null);
    ...
    public ItemServerPlayList getItem(int pos) {
        ...
        if(arrayList.get(pos) instanceof ItemServerPlayList){
            return (ItemServerPlayList) arrayList.get(pos);
        }else{
            return null;
        }
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint.toString().length() > 0) {
                ...
                for (int i = 0, l = filteredArrayList.size(); i < l; i++) {
                    String nameList = ((ItemServerPlayList)filteredArrayList.get(i)).getName();
                    if (nameList.toLowerCase().contains(constraint))
                        filteredItems.add((ItemServerPlayList) filteredArrayList.get(i));
                }
