...
public class AdapterOFSongList extends RecyclerView.Adapter<AdapterOFSongList.MyViewHolder> {
    ...
    private void openOptionPopUp(ImageView imageView, final int pos) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            popup.getMenu().findItem(R.id.popup_off_delete).setVisible(false);
        }
    ...
    private void setRingtone(final int pos, final String type) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            ...
            if (!settingsCanWrite) {
                ...
                setAsRingtone(arrayList.get(pos), type);
...
//            loadRingTone(pos, type);
    ...
    private void setAsRingtone(ItemSong itemSong, String type) {
        ...
        if(android.os.Build.VERSION.SDK_INT >= 29) {
            md.setDataSource(context, Uri.parse(itemSong.getUrl()));
        } else {
        ...
        if(android.os.Build.VERSION.SDK_INT >= 29) {
            ...
            newUri = context.getContentResolver().insert(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, content);
