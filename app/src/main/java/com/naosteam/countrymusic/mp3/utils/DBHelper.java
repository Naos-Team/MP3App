...
import com.naosteam.countrymusic.mp3.item.ItemAbout;
...
//        File root = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + context.getString(R.string.app_name) + "/temp");
    ...
    public void addtoAbout() {
        try {
            ...
            contentValues.put(TAG_ABOUT_CLICK, Constant.ad_interstitial_display);
    ...
    public Boolean getAbout() {
        ...
        if (c != null && c.getCount() > 0) {
            ...
            for (int i = 0; i < c.getCount(); i++) {
                ...
                Constant.ad_interstitial_display = Integer.parseInt(c.getString(c.getColumnIndex(TAG_ABOUT_CLICK)));
    ...
    public ArrayList<ItemSong> loadDataPlaylist(String pid, Boolean isOnline) {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            for (int i = 0; i < cursor.getCount(); i++) {
                ...
                if(!table.equals(TABLE_PLAYLIST_SONG_OFFLINE)) {
                    imagebig = methods.decrypt(cursor.getString(cursor.getColumnIndex(TAG_IMAGE)));
                    imagesmall = methods.decrypt(cursor.getString(cursor.getColumnIndex(TAG_IMAGE_SMALL)));
                } else {
                    imagebig = Uri.fromFile(new File(methods.decrypt(cursor.getString(cursor.getColumnIndex(TAG_IMAGE))))).toString();
                    imagesmall = Uri.fromFile(new File(methods.decrypt(cursor.getString(cursor.getColumnIndex(TAG_IMAGE_SMALL))))).toString();
                }
...
//                String imagesmall = methods.decrypt(cursor.getString(cursor.getColumnIndex(TAG_IMAGE_SMALL)));
                ...
                String imagebig = Uri.fromFile(new File(methods.decrypt(cursor.getString(cursor.getColumnIndex(TAG_IMAGE))))).toString();
                String imagesmall = Uri.fromFile(new File(methods.decrypt(cursor.getString(cursor.getColumnIndex(TAG_IMAGE_SMALL))))).toString();
